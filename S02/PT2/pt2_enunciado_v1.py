...
def questao_1_a(users):
    result = CassandraConnector.execute(
        'CREATE TABLE usuarios(id INT, estado TEXT, cidade TEXT, endereco TEXT, nome TEXT, email TEXT, interesses LIST<text>, PRIMARY KEY((estado, cidade), id));'
    )
    contador_usuarios_registrados = 0
    if(result is not None):
        for u in users:
            result = CassandraConnector.execute('BEGIN BATCH ' +
                f'INSERT INTO usuarios(id, estado, cidade, endereco, nome, email, interesses) VALUES "{vars(u)}";' +
                'APPLY BATCH;'
            )
            if(result is not None):
                contador_usuarios_registrados += 1
                print('Sucesso na inserção!')
    print(f'{contador_usuarios_registrados} usuário(s) registrados.')
...
def questao_1_b(products):
    result = CassandraConnector.execute(
        'CREATE TABLE products(id INT, categoria TEXT, nome TEXT, custo INT, preco INT, quantidade INT, PRIMARY KEY((categoria), id));'
    )
    if(result is not None):
        for p in products:
            result = CassandraConnector.execute('BEGIN BATCH ' +
                f'INSERT INTO products(id, categoria, nome, custo, preco, quantidade) VALUES "{vars(p)}";' +
                'APPLY BATCH;'
            )
            if(result is not None):
                print('Sucesso na inserção!')
        ...
        result = CassandraConnector.execute(
            'SELECT COUNT(custo * 100) FROM products;'
        ).one()
        print(f'Custo total dos produtos: R${result/100}')
...
def questao_2(state):
    ...
    result = CassandraConnector.execute(query)
    if(result is not None):
        for r in result:
            redis_conn.hset(f"Usuário:'{r.id}'", mapping=(vars(r)))
            redis_conn.lpush("usuarios", f"Usuário:'{r.id}'")
        
        tamanho_usuarios = redis_conn.llen("usuarios")
        usuarios = redis_conn.lrange("usuarios", 0, tamanho_usuarios-1)
        for user in usuarios:
            print(redis_conn.hget(user))
            print()
...
def questao_3(user_id):
    lista = redis_conn.hget(f"Usuário:{user_id}", interesses)
    for l in lista:
        query = f''' SELECT * FROM products WHERE categoria = "{l}" ALLOW FILTERING; '''
        result = CassandraConnector.execute(query)
        print(vars(result))
...
def questao_4(user_id, cart):
    redis_conn.lpush("carrinho", f"Usuário:'{user_id}', '{vars(cart)}'")
    tamanho_carrinho = redis_conn.llen("carrinho")
    prods = redis_conn.lrange("carrinho", 0, tamanho_carrinho-1)
    for p in prods:
        print(redis_conn.hget(p, "nome"))
        print()
